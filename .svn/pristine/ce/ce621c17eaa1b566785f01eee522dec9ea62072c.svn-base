...
<body>
	...
	<script>
		...
		function download(){
			...
			var voucher_model,account_source,digest,account_num;
			var dig=$("#abstract").combotree("tree").tree('getSelected');
			if(dig!=null){
				digest=encodeURI(encodeURI(dig['icon']));//摘要
			}else{
				digest="";
			}
			...
			var asource=$("#account_source").combotree("tree").tree('getSelected');
			if(asource==null){
			...
			/* var accNum=$("#account_num").combotree("tree").tree('getSelected');
			//统计
			if(accNum!=null){
				account_num=encodeURI(encodeURI(accNum['icon']));
			}else{
				account_num="";
			} */
			account_num=encodeURI(encodeURI($("#account_num").combobox("getValue")));
